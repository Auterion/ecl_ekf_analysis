...
from ecl_ekf_analysis.analysis.post_processing import (
...
from ecl_ekf_analysis.plotting.data_plots import (
    CheckFlagsPlot,
    ControlModeSummaryPlot,
    InnovationPlot,
    TimeSeriesPlot,
)
...
def create_pdf_report(ulog: ULog, output_plot_filename: str) -> None:
    ...
    ) = detect_airtime(control_mode, status_time)
    ...
    with PdfPages(output_plot_filename) as pdf_pages:
        ...
        data_plot = InnovationPlot(
            ekf2_innovations,
            [
                ('vel_pos_innov[2]', 'vel_pos_innov_var[2]'),
        ...
        data_plot = InnovationPlot(
            ekf2_innovations,
            [
                ('vel_pos_innov[0]', 'vel_pos_innov_var[0]'),
        ...
        data_plot = InnovationPlot(
            ekf2_innovations,
            [
                ('vel_pos_innov[3]', 'vel_pos_innov_var[3]'),
        ...
        data_plot = InnovationPlot(
            ekf2_innovations,
            [('flow_innov[0]', 'flow_innov_var[0]'), ('flow_innov[1]', 'flow_innov_var[1]')],
            x_labels=['time (sec)', 'time (sec)'],
        ...
        data_plot = CheckFlagsPlot(
            ...
            innov_flags,
            [
                ['vel_innov_fail', 'posh_innov_fail'],
                ['posv_innov_fail', 'hagl_innov_fail'],
                ['magx_innov_fail', 'magy_innov_fail', 'magz_innov_fail', 'yaw_innov_fail'],
                ['tas_innov_fail'],
                ['sli_innov_fail'],
        ...
        scaled_estimator_status = {
            'vibe[0]': 1000.0 * estimator_status['vibe[0]'],
            'vibe[1]': 1000.0 * estimator_status['vibe[1]'],
        ...
        data_plot = CheckFlagsPlot(
            ...
            x_label='time (sec)',
            y_labels=['Del Ang Coning (mrad)', 'HF Del Ang (mrad)', 'HF Del Vel (m/s)'],
        ...
        scaled_innovations = {
            'output_tracking_error[0]': 1000.0 * ekf2_innovations['output_tracking_error[0]'],
        ...
        data_plot = CheckFlagsPlot(
            ...
            x_label='time (sec)',
            y_labels=['declination (deg)', 'inclination (deg)', 'strength (Gauss)'],
...
def detect_airtime(control_mode, status_time):
    ...
    if np.amin(np.diff(control_mode['airborne'])) < 0.0:
        ...
        print('always on ground')
    if (
        np.amax(np.diff(control_mode['airborne'])) > 0.5
        and np.amin(np.diff(control_mode['airborne'])) < -0.5
    ):
