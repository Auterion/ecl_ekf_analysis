...
def compare_float_values(
    ...
    assert type(analysis_value) == type(ground_truth_value), (
        ...
        "in {:s} in {:s}".format(
            str(type(analysis_value)), str(type(ground_truth_value)), value_name, log_id
        )
    )
    if isinstance(ground_truth_value, float):
        assert analysis_value == pytest.approx(
            ground_truth_value, rel=rel_tol, abs=abs_tol
...
def compare_check_analysis_result_ground_truth(
    check_result: dict,
    ground_truth_check_result: dict,
    log_id: str,
    rel_tol: float = 1e-6,
    abs_tol: float = 1e-12,
) -> None:
    ...
    """
    assert (
        check_result["type"] == ground_truth_check_result["type"]
    ...
    assert (
        check_result["status"] == ground_truth_check_result["status"]
    ), "analysis check result status {:s} in {:s} differed from ground truth {:s} in {:s}".format(
        check_result["status"], check_result["type"], ground_truth_check_result["status"], log_id
    )
    ...
    for check_statistic, ground_truth_check_statistic in zip(
        ...
        assert (
            check_statistic["type"] == ground_truth_check_statistic["type"]
        ...
        assert (
            check_statistic["instance"] == ground_truth_check_statistic["instance"]
        ...
        compare_float_values(
            "{:s} failure threshold".format(check_statistic["type"]),
            log_id,
        ...
        compare_float_values(
            "{:s} warning threshold".format(check_statistic["type"]),
            log_id,
